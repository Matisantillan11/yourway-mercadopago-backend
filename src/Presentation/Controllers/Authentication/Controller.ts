...
export default class Controller implements Routeable, Patheable {
	...
	private initializeRoutes(validationProvider: Validateable) {
		...
		this.router.post(
			`${this.path}/register/:db`,
			[validationProvider.validate(this.userDto)],
			this.registration,
		)
		this.router.post(
			`${this.path}/loginverified`,
			[validationProvider.validate(this.loginDto)],
			this.loggingInVerified,
		)
		this.router.post(
			`${this.path}/login/:db`,
			[validationProvider.validate(this.loginDto)],
	...
	private validateUser = async (
		...
		try {
			...
			let verificationResponse: DataStoredInTokenable = (await jwt.verify(
				token,
				secret,
			)) as DataStoredInTokenable
			if (verificationResponse?._id && verificationResponse?.database) {
				...
				const model: Model<Document, {}> =
					await this.connectionProvider.getModel(database, schema.name, schema)
	...
	private requestResetPass = async (
		request: Request,
		response: Response,
		next: NextFunction,
	) => {
		try {
			...
			if (email) {
				...
				const existUser = await this.userService.existUserWithThatEmail(
					email,
					userModel,
				)
	...
	private resetPass = async (
		...
		let userModel: Model<Document, {}> = await this.connectionProvider.getModel(
			db,
			this.userSchema.name,
			this.userSchema,
		)
		...
		if (db && db != '') {
			...
			if (userResponse.result !== undefined) {
				const resetPasswordResponse = await this.authenticationService.getPass(
					userResponse.result.email,
					objData.pass,
					userId,
					userModel,
				)
				...
				this.responserService.res = {
					result: undefined,
					message:
						'El e-mail no se encuentra registrado, por favor verifica que esté escrito correctamente',
					status: 404,
					error: '',
				}
			}
	...
	private registration = async (
		...
		if (database && database != '') {
			try {
				const registerResponse: DomainResponseable =
					await this.authenticationService.register(userData, database, model)
	...
	private loggingIn = async (
		...
		if (database && database != '') {
			try {
				...
				const loginResponse: DomainResponseable =
	...
	private loggingInVerified = async (
		request: Request,
		response: Response,
		next: NextFunction,
	) => {
		...
		if (database && database != '') {
			try {
				...
				const loginResponse: DomainResponseable =
					await this.authenticationService.loginVerified(
						logInData,
						database,
						model,
					)
